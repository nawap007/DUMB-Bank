...
import com.zoom.cce.service.MailService;
import com.zoom.cce.service.UserService;
import com.zoom.cce.service.dto.ResponseWrapper;
import com.zoom.cce.service.dto.UserDTO;
import com.zoom.cce.web.rest.vm.ManagedUserVM;
import com.zoom.cce.web.rest.util.HeaderUtil;
import com.zoom.cce.web.rest.util.PaginationUtil;
...
public class UserResource {
	...
	public ResponseEntity createUser(@RequestBody ManagedUserVM managedUserVM) throws URISyntaxException {
		...
		if (userRepository.findOneByEmail(managedUserVM.getEmail()).isPresent()) {
			...
			// .headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "userexists",
			...
			BankAccount acc = userService.createUser(managedUserVM);

			return new ResponseEntity<BankAccount>(acc, HttpStatus.CREATED);
	...
	@Timed
	public ResponseWrapper getDetail(@PathVariable("acc") String accNumber) {

		
		return  userService.getUser(accNumber);
	}
